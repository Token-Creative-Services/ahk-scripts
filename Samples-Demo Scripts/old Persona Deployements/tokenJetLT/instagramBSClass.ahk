...
class InstagramBS{
	...
	x_firstPicLiker :=
	y_firstPicLiker :=
	;//The space between people in a list. For example, the list of people who like a pic
	y_listIncrement :=
	...
	time_parallelSpaceAppPageLoad := ;How long it takes parallel space to load app the app listing page during account switch
	time_maxClickFollowInterval := ;how quickly can you perform a click to follow aciton on users in a list
	time_IGToParallelSpaceAppSwitch := ;when clicking the parallel space menu item, how long it takes so switch apps
	...
	SetToJSniderLaptop() 
	{
		try {
			...
			This.time_parallelSpaceAppPageLoad := "3000"
			This.time_IGToParallelSpaceAppSwitch := "5000"
	...
	SetToCVollLaptop() 
	{
		try {
			...
			This.y_listIncrement := "60"
			This.time_FollowerListLoad := "2500"
			This.time_ProfilePageLoad := "2500"
			This.time_ProfilePageSwapLoad := "10000"
			This.time_PictureLoad := "1500"
			This.time_doubleClickWaitInverval := "150"
			This.time_goBackWait := "150"
			This.time_searchExplorePageLoad := "5000"
			This.time_homePageLoad := "3000"
			This.time_activityFeedPageLoad := "5000"
			This.time_loggedInProfileDropDownListLoad := "3000"
			This.time_similarProfilesDropDownLoad := "4500"
			This.time_SimilarProfileFeedClickFollowSlide := "600" ; Used as Test case in method validation (below)***
			This.time_parallelSpaceAppPageLoad := ""
			This.time_IGToParallelSpaceAppSwitch := ""
	...
	;~ ;Checks first pic liker on liking list and attempt to follow
	;~ checkFirstPicLikeFollowButton()
	;~ {
		;~ This.activateBlueStacksWindow()
		;~ PixelGetColor, expectedFollowClickLocation, % This.x_firstPicLiker, % This.y_firstPicLiker, RGB ;Look to see if the follow button is click-able
		;~ IfEqual, expectedFollowClickLocation,  % This.colour_followButtonAreaBackground ;if the colour is blue (to indicate we can follow), we click to follow
		;~ {
			;~ This.activateBlueStacksWindow()
			;~ MouseMove, % This.x_firstPicLiker, % This.y_firstPicLiker
			;~ MouseClick, left, % This.x_firstPicLiker, % This.y_firstPicLiker
			;~ return 1 ;return 1 (successful click)
		;~ }
		;~ else {
			;~ MouseMove, % This.x_firstPicLiker, % This.y_firstPicLiker
	...
	;~ checkIterativeFollowButtonInPicLikerList()
	;~ {
		;~ MouseGetPos, xpos, ypos ;get our starting position
		;~ yIncrement := ypos + This.y_listIncrement ;increment the spot 
		;~ MouseMove,  xpos, yIncrement ;move it there for visibility-sake
		;~ PixelGetColor, expectedFollowClickLocation, %xpos%, %yIncrement%, RGB ;Look to see if the follow button is click-able
		;~ IfEqual, expectedFollowClickLocation,  % This.colour_followButtonAreaBackground ;if the colour is blue (to indicate we can follow), we click to follow
		;~ {
			;~ MouseClick, left, %xpos%, %yIncrement%
			;~ Sleep, % This.time_parallelSpaceAppPageLoad
			;~ return 1 ;follow button was clicked
		;~ }
		;~ yIncrement := 0 ;reset this as method is called in loops often. We get the new value at the start of the next call.
		;~ return 0 ;follow button was not clicked
	;~ }
	;~ ;Extension of the above method, except calling it in a loop, delimited by the # of users in the on-screen list at one time
	;~ loopCheckIterativeFollowButtonInPicLikerList()
	;~ {
		;~ Loop 10 { 
			;~ This.checkIterativeFollowButtonInPicLikerList()
		;~ }
	;~ }
